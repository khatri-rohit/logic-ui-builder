const hardcodedColorRule = {
  meta: {
    ...
    messages: {
      noHardcodedColors:
        "Use design tokens (var(--surface), var(--primary)) instead of hardcoded colors like '{{color}}'",
    },
  },
  create(context) {
    return {
      JSXAttribute(node) {
        if (
          ...
          const hardcodedColors =
            /\b(bg|text|border|ring|from|to|via|placeholder|accent|caret|fill|stroke)-(blue|red|green|yellow|orange|purple|pink|indigo|cyan|teal|gray|slate|neutral|stone)-(50|100|200|300|400|500|600|700|800|900|950)\b/;
...
const arbitrarySpacingRule = {
  meta: {
    ...
    messages: {
      noArbitrarySpacing:
        "Use 8pt grid spacing (gap-2, gap-4, gap-6, gap-8) instead of '{{value}}'",
    },
  },
  create(context) {
    return {
      JSXAttribute(node) {
        if (
          ...
          const arbitrarySpacing =
            /\b(p|m|gap|px|py|mx|my)-(?!(?:1|2|3|4|6|8|10|12|14|16|20|24|28|32)\b)\d+\b/;
